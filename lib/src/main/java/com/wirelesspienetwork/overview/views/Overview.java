...
public class Overview extends FrameLayout implements OverviewStackView.Callbacks, OverviewStackView.ScrollerCallBacks {
    ...
    public void setTaskStack(OverviewAdapter adapter) {
        ...
        mStackView.animate().alpha(1.f).setStartDelay(500).setDuration(2500).start();

        //将滑动监听传出到main
        mStackView.setScrollerCallBack(this);
    ...
    public void onItemClick(View view, int position) {
        if (mCallbacks != null) {
            mCallbacks.onItemClick(view,position);
        }
    }
