...
public class OverviewStackView extends FrameLayout implements OverviewAdapter.Callbacks, OverviewStackViewScroller.Callbacks,
        ObjectPool.ObjectPoolConsumer<ViewHolder, Integer>,OverviewCard.OnItemClickListener {

    @Override
    public void onItemClick(View view, int position) {
        if(mCb != null){
            mCb.onItemClick(view,position);
        }
    }
    ...
    //添加滑动监听的回调接口
    interface ScrollerCallBacks{
        public void onScrollChanged(float f);
    }
    private ScrollerCallBacks scrollerCallbacks;

    public void setScrollerCallBack(ScrollerCallBacks scrollerCallBacks){
        this.scrollerCallbacks = scrollerCallBacks;
    }
    ...
    public void prepareObjectToEnterPool(ViewHolder vh) {
        Log.d("print", "prepareObjectToEnterPool: ");
    ...
    @Override
    public void prepareObjectToLeavePool(ViewHolder vh, final Integer position, boolean isNewView) {
        // Rebind the task and request that this task's data be filled into the TaskView

        Log.d("print", "prepareObjectToLeavePool:  ");
        ...
        if (isNewView) {
            ...
            //添加一个传入position的方法
            container.bindPosition(position);
            //设置点击回调
    ...
    public void onScrollChanged(float p) {
         ...
         * 添加滚动回调
         */
        if(scrollerCallbacks != null){
            scrollerCallbacks.onScrollChanged(p);
        }
